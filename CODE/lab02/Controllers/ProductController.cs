...
namespace lab02.Controllers
{
    public class ProductController : Controller
    {
        ...
        // action trả về một view là một trang html và nó có thể truyền tham số model
        public ViewResult TestViewResult()
        {
            return View();
        }
        //Actin trả về một PảtialViewResult
        ...
        {
            string js = "funtion checkEMail(){var btloc=/^([w-]+(?:.[w-]+)*)@((?:[w-]+.)*w[w-]{ 0,66}).([a - z]{ 2,6} (?:.[a - z]{ 2})?)$/ i if (btloc.test(mail)) { kq = true; } else { alert(“Emailaddress invalid”); kq = false; } return kq; }";
